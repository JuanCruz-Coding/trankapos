-- =====================================================================
-- Migration 042: create_sale_atomic con surcharge (Sprint PMP)
-- =====================================================================
-- Adapta create_sale_atomic para manejar recargos por método.
-- Payload de payments ahora puede incluir:
--   { method, amount, surcharge_amount?, method_config_id? }
-- Validación: sum(payments.amount) == subtotal - discount + surcharge_total.
-- =====================================================================
...
begin
  ...
  for v_payment in select * from jsonb_array_elements(p_payments) loop
    ...
    v_surcharge_total := v_surcharge_total + coalesce((v_payment->>'surcharge_amount')::numeric, 0);
    if (v_payment->>'method') = 'on_account' then
      v_on_account := v_on_account + (v_payment->>'amount')::numeric;
    end if;
  end loop;

  v_total := round(v_subtotal - coalesce(p_discount, 0) + v_surcharge_total, 2);
  ...
  insert into sales (
    ...
    subtotal, discount, total, surcharge_total, voided, status, stock_reserved_mode,
    ...
    v_subtotal, coalesce(p_discount, 0), v_total, v_surcharge_total, false, v_status, v_stock_mode,
  ...
  insert into sale_payments (sale_id, tenant_id, method, amount, surcharge_amount, method_config_id)
  select v_sale_id, p_tenant_id,
         (pay->>'method')::payment_method,
         (pay->>'amount')::numeric,
         coalesce((pay->>'surcharge_amount')::numeric, 0),
         nullif(pay->>'method_config_id', '')::uuid
